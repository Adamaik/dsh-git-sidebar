window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 提交消息草稿持久化：输入或生成的内容在关闭面板、刷新页面后保留。
    const MSG_KEY = "dsh-git-sidebar.msg";
    function loadMsg() {
      try {
        return window.localStorage.getItem(MSG_KEY) || "";
      } catch (e) {
        return "";
      }
    }
    function saveMsg(v) {
      try {
        window.localStorage.setItem(MSG_KEY, v);
      } catch (e) {}
    }
    ...
    function GitPanel(props) {
      ...
      const [msg, setMsg] = react.useState(loadMsg);
      ...
      function commit() {
        doOp(
          "提交",
          () => api("commit", { message: msg, all, cwd: cwd }),
          () => {
            // 提交成功后清空草稿，避免旧消息重复提交
            setMsg("");
            saveMsg("");
...
.dshgit-fab.active{color:var(--dsw-alias-state-business-primary,#4c8dff);background:rgba(76,141,255,.14);border-color:var(--dsw-alias-state-business-primary,#4c8dff)}
.dshgit-fab-label{writing-mode:vertical-rl;font-size:10.5px;letter-spacing:2px;font-weight:600;pointer-events:none}
.dshgit-scrim{position:fixed;inset:0;pointer-events:auto;background:var(--dsw-alias-bg-mask-1,rgba(0,0,0,.35))}
...
.dshgit-close:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.15));color:var(--dsw-alias-label-primary,#e6e8eb)}
...
.dshgit-branch-chip:hover{border-color:var(--dsw-alias-state-business-primary,#4c8dff);color:var(--dsw-alias-state-business-primary,#7aa5ff)}
...
.dshgit-dropdown{position:absolute;top:26px;left:0;z-index:10;min-width:220px;max-width:320px;max-height:260px;overflow-y:auto;background:var(--dsw-specific-menu,#26272b);border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.16));border-radius:8px;box-shadow:0 10px 30px rgba(0,0,0,.4);padding:4px}
...
.dshgit-drop-item:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.15))}
.dshgit-drop-item.active{color:var(--dsw-alias-state-business-primary,#7aa5ff);font-weight:600}
...
.dshgit-btn{background:var(--dsw-alias-bg-layer-2,#26272b);border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.12));color:var(--dsw-alias-label-primary,#e6e8eb);border-radius:6px;padding:4px 11px;font-size:12px;cursor:pointer}
.dshgit-btn:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.18))}
.dshgit-btn:disabled{opacity:.45;cursor:default}
.dshgit-btn-primary{background:var(--dsw-alias-button-primary-fill,#4c8dff);border-color:transparent;color:var(--dsw-alias-label-primary-foreground,#fff);font-weight:600}
.dshgit-btn-primary:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover,#3a70cc)}
.dshgit-btn-danger{border-color:var(--dsw-alias-state-warn-primary,#e5a50a);color:var(--dsw-alias-state-warn-primary,#e5a50a)}
.dshgit-btn-gen{border-color:var(--dsw-alias-state-business-primary,#4c8dff);color:var(--dsw-alias-state-business-primary,#7aa5ff)}
...
.dshgit-file:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.12))}
.dshgit-file.staged{opacity:.82}
.dshgit-badge{display:inline-flex;align-items:center;justify-content:center;min-width:22px;height:18px;border-radius:4px;font-size:10.5px;font-weight:700;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:var(--dsw-alias-interactive-bg-hover-accent,rgba(127,127,127,.22));color:var(--dsw-alias-label-secondary,#c8ccd2)}
...
.dshgit-badge.R,.dshgit-badge.C{color:var(--dsw-alias-state-business-primary,#7aa5ff)}
...
.dshgit-msg:focus{border-color:var(--dsw-alias-state-business-primary,#4c8dff)}
...
.dshgit-toggle:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.15));color:var(--dsw-alias-label-primary,#e6e8eb)}
.dshgit-toggle.active{color:var(--dsw-alias-state-business-primary,#7aa5ff);background:var(--dsw-alias-interactive-bg-hover-accent,rgba(76,141,255,.15))}
`;
      ctx.effect(() => {
